...
from snne.uncertainty.utils.eval_utils import (
    is_binary_list,
    bootstrap, 
...
def init_wandb(wandb_runid, assign_new_wandb_id, experiment_lot, entity):
    ...
    run_name = f"analyze_results_{wandb_runid}"
    ...
    if not assign_new_wandb_id:
        ...
        old_run = api.run(f'{entity}/snne/{wandb_runid}')
...
def analyze_run(
    ...
    eval_metrics = dict(zip(
        ['AUROC', 'area_under_thresholded_accuracy', 'AUARC', 'PRR', 'mean_uncertainty'],
        list(zip(
            [auroc, area_under_thresholded_accuracy, auarc, aucpr, np.mean],
            [compatible_bootstrap, compatible_bootstrap, compatible_bootstrap, compatible_bootstrap, bootstrap]
        )),
    ))
    ...
    for measure_name, measure_values in rum.items():
        ...
        for validation_is_false, logging_name in zip(validation_is_falses, logging_names):
            ...
            fargs = {
                ...
                'AUARC': [measure_values, validation_accuracy],
                'PRR': [measure_values, validation_accuracy],
            ...
            for fname, (function, bs_function) in eval_metrics.items():
                # If the target is not binary, don't compute AUROC
                if not is_binary and fname == 'AUROC':
                    print("Not a binary value. Ignore AUROC.")
                    continue
